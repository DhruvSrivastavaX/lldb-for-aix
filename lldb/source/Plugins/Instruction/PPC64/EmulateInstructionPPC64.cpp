...
EmulateInstructionPPC64::GetRegisterInfo(RegisterKind reg_kind,
  ...
  if (reg_kind == eRegisterKindGeneric) {
    ...
    case LLDB_REGNUM_GENERIC_PC:
      ...
      reg_num = gpr_pc_ppc64;
      break;
    case LLDB_REGNUM_GENERIC_SP:
      reg_kind = eRegisterKindLLDB;
      reg_num = gpr_r1_ppc64;
      break;
    case LLDB_REGNUM_GENERIC_RA:
      reg_kind = eRegisterKindLLDB;
      reg_num = gpr_lr_ppc64;
      break;
    case LLDB_REGNUM_GENERIC_FLAGS:
      reg_kind = eRegisterKindLLDB;
      reg_num = gpr_cr_ppc64;
      break;
...
bool EmulateInstructionPPC64::CreateFunctionEntryUnwind(
  ...
  row.GetCFAValue().SetIsRegisterPlusOffset(gpr_r1_ppc64, 0);
  ...
  unwind_plan.SetReturnAddressRegister(gpr_lr_ppc64);
  return true;
}
...
bool EmulateInstructionPPC64::EvaluateInstruction(uint32_t evaluate_options) {
  ...
  if (auto_advance_pc) {
    orig_pc_value =
        ReadRegisterUnsigned(eRegisterKindLLDB, gpr_pc_ppc64, 0, &success);
    if (!success)
      return false;
  ...
  if (auto_advance_pc) {
    ...
    if (new_pc_value == orig_pc_value) {
      ...
      context.SetNoArgs();
      if (!WriteRegisterUnsigned(context, eRegisterKindLLDB, gpr_pc_ppc64,
                                 orig_pc_value + 4))
        return false;
    }
  }
...
bool EmulateInstructionPPC64::EmulateMFSPR(uint32_t opcode) {
  ...
  bool success;
  uint64_t lr =
      ReadRegisterUnsigned(eRegisterKindLLDB, gpr_lr_ppc64, 0, &success);
  if (!success)
    return false;
  Context context;
  context.type = eContextWriteRegisterRandomBits;
  WriteRegisterUnsigned(context, eRegisterKindLLDB, gpr_r0_ppc64, lr);
...
bool EmulateInstructionPPC64::EmulateLD(uint32_t opcode) {
  ...
  if (ra != gpr_r1_ppc64 || rt != gpr_r1_ppc64 || ids != 0)
    return false;
  ...
  std::optional<RegisterInfo> r1_info =
      GetRegisterInfo(eRegisterKindLLDB, gpr_r1_ppc64);
  if (!r1_info)
    return false;
  ...
  WriteRegisterUnsigned(ctx, eRegisterKindLLDB, gpr_r1_ppc64, 0);
...
bool EmulateInstructionPPC64::EmulateSTD(uint32_t opcode) {
  ...
  if (rs != gpr_r1_ppc64 && rs != gpr_r31_ppc64 && rs != gpr_r30_ppc64 &&
      rs != gpr_r0_ppc64)
    return false;
...
bool EmulateInstructionPPC64::EmulateOR(uint32_t opcode) {
  ...
  if (m_fp != LLDB_INVALID_REGNUM || rs != rb ||
      (ra != gpr_r30_ppc64 && ra != gpr_r31_ppc64) || rb != gpr_r1_ppc64)
    return false;
...
bool EmulateInstructionPPC64::EmulateADDI(uint32_t opcode) {
  ...
  if (rt != gpr_r1_ppc64 || ra != gpr_r1_ppc64)
    return false;
  ...
  bool success;
  uint64_t r1 =
      ReadRegisterUnsigned(eRegisterKindLLDB, gpr_r1_ppc64, 0, &success);
  if (!success)
    return false;
  WriteRegisterUnsigned(ctx, eRegisterKindLLDB, gpr_r1_ppc64, r1 + si_val);
  LLDB_LOG(log, "EmulateADDI: success!");

  // FIX the next-pc
  uint64_t pc_value = ReadRegisterUnsigned(eRegisterKindLLDB, gpr_pc_ppc64, 0, &success);
  uint64_t next_pc = pc_value + 4;
  ctx.type = eContextAdjustPC;
  WriteRegisterUnsigned(ctx, eRegisterKindLLDB, gpr_pc_ppc64, next_pc);
...
bool EmulateInstructionPPC64::EmulateBC(uint32_t opcode) {
  ...
  bool success;
  uint64_t ctr_value = ReadRegisterUnsigned(eRegisterKindLLDB, gpr_ctr_ppc64, 0, &success);
  if ((~BO) & (1U << 2))
    ctr_value = ctr_value - 1;
  bool ctr_ok = (bool)(BO & (1U << 2)) | ((bool)(ctr_value != 0) ^ (bool)(BO & (1U << 1)));
  uint64_t cr_value = ReadRegisterUnsigned(eRegisterKindLLDB, gpr_cr_ppc64, 0, &success);
...
bool EmulateInstructionPPC64::EmulateBCLR(uint32_t opcode) {
  ...
  uint64_t next_pc = pc_value + 4;
  if (ctr_ok & cond_ok) {
    next_pc = ReadRegisterUnsigned(eRegisterKindLLDB, gpr_lr_ppc64, 0, &success);
    next_pc &= ~((1UL << 2) - 1);
  }
...
bool EmulateInstructionPPC64::EmulateBCCTR(uint32_t opcode) {
  ...
  uint64_t next_pc = pc_value + 4;
  if (cond_ok) {
    next_pc = ReadRegisterUnsigned(eRegisterKindLLDB, gpr_ctr_ppc64, 0, &success);
    next_pc &= ~((1UL << 2) - 1);
...
bool EmulateInstructionPPC64::EmulateBA(uint32_t opcode) {
  ...
  bool success;
  uint64_t next_pc = ReadRegisterUnsigned(eRegisterKindLLDB, gpr_lr_ppc64, 0, &success);
