...
#ifndef LLDB_SOURCE_PLUGINS_INSTRUCTION_PPC_EMULATEINSTRUCTIONPPC_H
#define LLDB_SOURCE_PLUGINS_INSTRUCTION_PPC_EMULATEINSTRUCTIONPPC_H
...
class EmulateInstructionPPC : public EmulateInstruction {
public:
  EmulateInstructionPPC(const ArchSpec &arch);

  static void Initialize();

  static void Terminate();

  static llvm::StringRef GetPluginNameStatic() { return "ppc"; }
...
private:
  struct Opcode {
    uint32_t mask;
    uint32_t value;
    bool (EmulateInstructionPPC::*callback)(uint32_t opcode);
    const char *name;
  };
