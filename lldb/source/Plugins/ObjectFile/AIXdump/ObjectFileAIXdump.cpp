...
LLDB_PLUGIN_DEFINE(ObjectFileAIXdump)

void ObjectFileAIXdump::Initialize() {
  PluginManager::RegisterPlugin(
      GetPluginNameStatic(), GetPluginDescriptionStatic(), CreateInstance,
      CreateMemoryInstance, GetModuleSpecifications, SaveCore);
}

void ObjectFileAIXdump::Terminate() {
  PluginManager::UnregisterPlugin(CreateInstance);
}

ObjectFile *ObjectFileAIXdump::CreateInstance(
...
ObjectFile *ObjectFileAIXdump::CreateMemoryInstance(
    const lldb::ModuleSP &module_sp, WritableDataBufferSP data_sp,
    const ProcessSP &process_sp, lldb::addr_t header_addr) {
  return nullptr;
}

size_t ObjectFileAIXdump::GetModuleSpecifications(
  ...
  Log *log = GetLog(LLDBLog::Process);
    LLDB_LOGF(log, "GOT HERE!!! %d", initial_count);
...
bool ObjectFileAIXdump::SaveCore(const lldb::ProcessSP &process_sp,
  ...
  // AIXdump defaults to stacks only.
